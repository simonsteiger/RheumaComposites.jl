...
function valid_joints(x; max=28)
    0 <= x <= max || error(x, "joints must be between 0 and $max.")
end
...
function valid_vas(::Real)
    error("VAS must be a length. Forgot to convert? See Unitful.@u_str")
end
...
function valid_apr(::Real) 
    error("APR must be a concentration. Forgot to convert? See Unitful.@u_str")
end
