...
weight(::IsUnweighted, x::T) where {T} = x.components

map_weights(weights, x) = map((w, v) -> w(v), values(weights), x.components)
...
weight(::IsWeighted, x::Partial{DAS28CRP}) = map_weights(weights_das28crp, x)
=#
